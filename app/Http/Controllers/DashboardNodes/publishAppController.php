...
class publishAppController extends Controller
{
...
#[
  ...
  $datumOne = User::where([
    ...
    ['registrationPurpose', 'Business'],

    ['appActiveStageURL', '/application/dashboard/publish'],
...
#]
          ...
          return redirect($user_id->appActiveStageURL);
...
#]
  ...
  return view('dashboard.publishApp');
  ...
  public function publishApp(Request $request){
  		...
  		$request->validate([
  			...
  			'keywordOne' => 'required|max:45|min:3',
  			...
  			'coverPhoto' => 'required|image', /// check for max size later 10000
  			...
  			'PhotoNine' => 'nullable|image', /// check for max size later
                 ...
                 if($request->hasFile('PhotoOne')){
  				 ...
  				 if($request->hasFile('PhotoFour')){
  					...
  					$extensionPhotoNine = $request->file('PhotoNine')->getClientOriginalExtension();
